import {
  ...
  ISSDetailsModal,
  SafetyReminderModal,
  ARTutorialModalItemsLayout,
  ARTutorialModal,
  arTutorialTotalStages,
} from "../../components"
...
export const ISSViewScreen = observer(function ISSNowScreen() {
  ...
  const [tutorialItemsLayout, setTutorialItemsLayout] = useState<ARTutorialModalItemsLayout>({})
  ...
  useEffect(() => {
    if (
      ...
      isActive &&
      Object.keys(tutorialItemsLayout).length === arTutorialTotalStages
    ) {
  ...
  const handleTutorialItemLayout = useCallback(
    (item: keyof ARTutorialModalItemsLayout) => (event: LayoutChangeEvent) => {
      event.persist()
      setTimeout(() => {
    ...
    >
      ...
      >
        {isTutorialFullyVisible && (
          <ARTutorialModal itemsLayout={tutorialItemsLayout} onComplete={handleSetCoachCompleted} />
        )}
      </ModalContainer>
      ...
      >
        <SafetyReminderModal
          ...
          onBack={handleSafetyReminderBack}
        />
      </ModalContainer>
    </Screen>
  )
