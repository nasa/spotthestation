...
export class NightOverlay {
  ...
  constructor() {
    ...
    this.getShadowPosition = () => {
      ...
      return sunPosition
        ? { latitude: -sunPosition.latitude, longitude: sunPosition.longitude + 180 }
        : null
    }
    ...
    this.calculatePositionOfSun = () => {
      ...
      const sunEq =
        Math.sin(rad * meanAnomSun) * (1.914602 - jc * (0.004817 + 0.000014 * jc)) +
      ...
      const rqOfTime =
        ...
        ((y * Math.sin(2 * rad * meanLongSun) -
          ...
          1.25 * eccent * eccent * Math.sin(2 * rad * meanAnomSun)) /
          rad)
      const trueSolarTimeInDeg = ((msPastMidnight + rqOfTime * 60000) % 86400000) / 240000

      const longitude = -(trueSolarTimeInDeg < 0
