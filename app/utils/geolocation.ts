...
export const getNearbyPlaces = async ({ lng, lat }: Point, radius: number): Promise<LocationType[]> => {
  ...
  if (res.kind === "ok") {
    for (const googlePlace of res.places) {
      places.push({ location: googlePlace.geometry.location, title: googlePlace.name, subtitle: googlePlace.vicinity, alert: true })
    }
  }
...
export const getPlaces = async (search: string): Promise<LocationType[]> => {
  const places: LocationType[] = []
  const res = await api.getPlaces(`https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${search.replaceAll(' ', '%20')}&inputtype=textquery&fields=formatted_address%2Cname%2Cgeometry&key=${Config.GOOGLE_API_TOKEN}`,
    "candidates"
  ) 
