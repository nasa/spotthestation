const ja = {
  ...
  thanksModal: {
    body: "ご連絡いただきありがとうございます。 メッセージを受信し、リクエストを処理させていただきます。 このアプリケーションはユーザーデータを収集しないため、すべてのメッセージに個別に返信することはできないことにご注意ください。よくある質問ページをご覧いただき、ご質問への回答があるかどうかをご確認ください。",
    dismiss: "終了",
    faq: "よくある質問",
  },
  privacy: {
  ...
  homeScreen: {
    ...
    coachMarks: {
      ...
      sightingsData:
        "このセクションでは、選択した位置の近くで次にISSが発見される日付/時刻とカウントダウンを表示します。発見情報ボックスをタップすると、最近の過去の発見情報と今後の発見情報の完全なリストを見ることができます。",
  ...
  issView: {
    ...
    details: {
      ...
      orbitalPeriod: "軌道期間",
      orbitsPerDay: "軌道/日",
      dimensionsValue: "幅{{width}} x長さ{{length}} x高さ{{height}}",
      dateTime: "日付と時刻",
      maxHeight: "最大高度",
      duration: "地平線上の持続時間",
      appears: "出現",
  ...
  settings: {
    ...
    contactUsData: {
      ...
      sendButton: "送信",
      reportIssueHint:
        "サポートチームが報告された問題をより正確に再現し診断できるよう、問題の説明に加えて、選択した場所およびステーションの観測機会の日時を含めてください。",
      contactUsOptions: {
  ...
  resources: {
    ...
    faq: {
      ...
      questions: {
        ...
        answer3:
          "国際宇宙ステーションは太陽光を反射するために見えます。これは月が見える理由と同じです。しかし、月とは異なり、宇宙ステーションは昼間に見るには十分な明るさではありません。観測の機会は、月に1回から週に数回まで変わることがあります。これは、太陽の光がステーションに反射し、あなたの場所で夜明けと夕暮れに上空を通過するためです。<br/>" +
          "モバイルアプリは、今後14日間までの観測機会を計算して表示します。2026年5月以降、NASAのSpot the Stationウェブサイトでは、アプリを使わずにブラウザから直接ステーションの観測機会を見つける機能が復活しました：<a href='https://www.nasa.gov/spot-the-station/'>https://www.nasa.gov/spot-the-station/</a> 国と地域・都市を選択すると、今後14日間までの観測機会を確認できます。",
        ...
        answer6:
          "Spot the Stationアプリは、国際宇宙ステーションの次の通過についてユーザーに通知するためにプッシュ通知を送信します。ユーザーは、デバイスの設定でアプリの通知許可が有効になっていることを確認する必要があります。アプリは選択した場所の観測機会を今後14日間まで計算するため、しばらくアプリを使用しないと通知が届かなくなる場合があることにご注意ください。",

        question7: "7. Spot the Stationアプリで通知をカスタマイズできますか？",
        answer7:
          "Spot the Stationアプリには、ユーザーの好みの場所とアラートのタイミングに特化したプッシュ通知を受け取るための個別のアラート設定機能があります。通知設定はアプリの設定ページにあり、ユーザーはすべての今後のイベントの通知をオンにするか、現在選択されている場所の通知をカスタマイズできます。ユーザーは、場所設定を通じて他の場所の通知をカスタマイズできます。ホームページから観測ページにアクセスして、選択した場所の今後の各観測機会の通知をオン/オフに切り替えることができます。",

        question8: "8. 通知を受け取らない場合はどうすればよいですか？",
        answer8:
          "ユーザーは、Spot the Stationアプリの通知設定（設定ページの通知設定）を確認して、デバイスが好みの場所とタイミングでアラートを受け取るように設定されていることを確認する必要があります。ユーザーがまだアラートを受け取らない場合は、デバイスの設定で通知が有効になっていることを確認する必要があります。アプリは、開くたびに選択した場所の観測機会を今後14日間まで計算するため、定期的に確認しないと通知が届かなくなる場合があることにご注意ください。",
        ...
        answer10:
          "あなたの場所で暗くて良好な視界が必要であり、宇宙ステーションが上空にある必要があります。宇宙ステーションの軌道は地球全体を回るため、昼間や、地球の影に隠れる夜中に見えない時間に上空を通過することがあります。Spot The Stationは、あなたの場所で国際宇宙ステーションを観測する機会があるときにのみ通知を送信し、毎回上空を通過するたびに通知を送信するわけではありません。観測機会は最大14日先まで計算されるため、新たに予測された機会を確認するために定期的にチェックしてください。",
        ...
        answer12:
          "国際宇宙ステーションは、月と同じように太陽光を反射するために見えます。ステーションは地球の周りを移動するにつれて、地球の影に入ったり出たりします。地球の影に入ると、たとえ上空を周回していても見えなくなります。",
        ...
        answer14:
          "各観測について、Spot the Stationアプリは時間、地平線上での視認可能な持続時間、地平線上の最大高度、国際宇宙ステーションが現れたり消えたりする方向、および観測時の選択した場所の予報雲量を表示し、ユーザーが空で正確に位置を特定するのを助けます。",
        ...
        answer17:
          "はい、Spot the Stationアプリには、地球を周回する国際宇宙ステーションの現在の位置を示すリアルタイムマップが含まれており、ユーザーがその進行を追跡するための視覚的な参考になります。ステーションの軌道を2D、3D、または衛星ビューで見るには、トラッカーページにアクセスしてください。",

        question18: "18. Spot the Stationアプリの拡張現実機能とは何ですか？",
        answer18:
          "Spot the Stationアプリの拡張現実機能は、ユーザーが電話のカメラビューの上に重ねて、空における国際宇宙ステーションの経路の仮想オーバーレイを表示できるようにします。この機能は、ユーザーがデバイスをステーションのリアルタイムの位置に合わせることで、ステーションをより正確に見つけるのを助けます。",
        ...
        answer20:
          "Spot the Stationの拡張現実機能には、3D空間での向きを判断できるデバイスが必要です。ジャイロスコープやモーションコプロセッサなどの特定のハードウェアサポートが必要です。古いまたは低価格のデバイスは、磁力計センサーの制限により、この機能をサポートしていない場合があります。アプリの設定メニューにあるコンパスキャリブレーションガイドに従うことで、精度が向上する場合があります。",

        question21: "21. 拡張現実機能はどのように機能しますか？",
        answer21:
          "デバイスのカメラとセンサーを使用して、Spot the Stationの拡張現実機能は、国際宇宙ステーションの位置を空に重ね合わせ、ユーザーがデバイスを動かすとリアルタイムで調整します。アプリは、デバイスのカメラを正しい方向に向けるようにユーザーをガイドし、ステーションがどこに現れ、消えるかを示します。アプリの設定メニューからARビューのチュートリアルに従ってください。",
        ...
        answer23:
          "はい、Spot the Stationアプリ内の拡張現実機能は、デバイスのGPS位置に基づいて正確な位置情報を提供するように設計されています。ただし、デバイスのコンパスとセンサーのキャリブレーションに応じて、精度がわずかに異なる場合があります。ユーザーが不一致を感じた場合は、アプリの設定メニューにあるコンパスキャリブレーションガイドに従ってデバイスのコンパスを再キャリブレーションしてください。古いまたは低価格のデバイスは、磁力計センサーの制限により、この機能をサポートしていない場合があります。",
        ...
        answer25:
          "Spot the Stationアプリでの最適な拡張現実体験のために、空がよく見える開けた場所で機能を使用してください。高い建物や木などの障害物を避けてください。これらは視認性を妨げる可能性があります。デバイスのコンパスをキャリブレーションし（設定メニューのコンパスキャリブレーションガイドに従ってください）、位置情報サービスとカメラの許可がスムーズな動作のために有効になっていることを確認してください。",

        question26: "26. 拡張現実機能はiOSとAndroidデバイスの両方で利用できますか？",
        answer26:
          "はい、Spot the Stationアプリの拡張現実機能は、デバイスがハードウェア要件を満たしている限り、iOSおよびAndroidのモバイルおよびタブレットデバイスで利用可能です。古いまたは低価格のデバイスは、磁力計センサーの制限により、この機能をサポートしていない場合があります。",
        ...
        answer30:
          "Spot the Stationアプリのサポートについては、アプリの設定メニューにあるお問い合わせオプションを通じて連絡するか、<a href='mailto:hq-spotthestation@mail.nasa.gov'>Spot the Stationチームにメールを送信</a>してください。",
      },
    },
    astronauts: {
      title: "今ステーションにいるのは誰",
